...
def rec_loss_norm4D(x, mean, var):
    return -torch.mean(
        torch.sum(-0.5 * ((x - mean) ** 2 / var + torch.log(var) + torch.log(torch.tensor(2 * torch.pi))), dim=(1, 2, 3))
    )

def rec_loss_norm2D(x, mean, var):
    return -torch.mean(
        torch.sum(-0.5 * ((x - mean) ** 2 / var + torch.log(var) + torch.log(torch.tensor(2 * torch.pi))), dim=1)
    )
...
class resblock_enc(nn.Module):
    def __init__(self, in_channels, out_channels, pooling_size):
        ...
        self.bypass_conv = nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=True)
...
def plot_frf(ids, chs, model, loader, dlf = 0.005):
    ...
    fig, ax = plt.subplots(nrow, ncol, figsize = (ncol * 2.5, nrow * 1.5), tight_layout = True, sharex = True, sharey = True)
    if nrow == 1:
        ax = ax[None,:]
    if ncol == 1:
        ax = ax[:,None]
    for i in range(nrow):
        for j in range(ncol):
            ...
            ax[i, j].plot(f_axis, (y_rec + y_rec_var)[ids[j], chs[i], 0, :], linestyle = 'dashed', color = 'orange')
            ax[i, j].plot(f_axis, (y_rec - y_rec_var)[ids[j], chs[i], 0, :], linestyle = 'dashed', color = 'orange')
            ax[i, j].plot(f_axis, (y_pre + y_pre_var)[ids[j], chs[i], 0, :], linestyle = 'dashed', color = 'royalblue')
            ax[i, j].plot(f_axis, (y_pre - y_pre_var)[ids[j], chs[i], 0, :], linestyle = 'dashed', color = 'royalblue')
            if i == nrow - 1:
