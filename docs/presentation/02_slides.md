...
- **観測情報の制約**による **解の非一意性** — 異なるパラメータ配分が同じ応答を出すケースがあり、点推定は **複数の解候補を見落とす**
- **精度・不確実性** がわからない
...
→ これを回避できる **潜在空間ベイズ推論 (LSBI)** が近年提案されている (Yaoyama et al., 2026)
...
![w:960 center](assets/mvae_io.png)

- **MVAE: 2encoder + 1decoder**、観測 $x$（周波数応答関数）とパラメータ $\theta$ が **共有潜在空間$z$** を経由して対応
- **学習**: 双方向 KL + 再構成損失 → 2 エンコーダ出力が同じ潜在空間で **重なる** よう整列
- **学習後の使い道**: 学習済みエンコーダ $q_{\phi_x}, q_{\phi_\theta}$ が、そのまま **式 (2) の被積分関数として近似尤度 $\hat{L}$ の計算に投入される**

<!--
SPEAKER: 30秒。左の入力(FRF, θ) → エンコーダ → 潜在空間で重なる → デコーダ、と図を指で追う。最後に「学習後に2つのエンコーダがそのまま式(2)に入る」で締める。
-->
...
(yaoyama et al., 2026) で採用されている具体手法を、**共通の概念的役割** に分離して再定義:
...
**ベンチマーク** (Yaoyama et al., 2026 と同じ題材):

- 4 自由度せん断建物の **層剛性同定問題**
- 観測は **屋上の1観測点の周波数応答関数** (1024 次元) のみ
...
→ **同一応答を与える別パラメータ解** を「単一解に押し込めず、事後分布で正直に表現」できている
