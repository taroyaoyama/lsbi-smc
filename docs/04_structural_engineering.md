...
【入力】屋根のFRF（1024次元）
  ...
  1. enc_x で MVAE 潜在空間 z に写像（8次元、一度だけ実施）
         ↓
  2. SMC で事後分布を推定（粒子は無制約空間 θ_latent ∈ ℝ⁴ でサンプリング）
         ↓ 各粒子（候補θ）に対して：
         └─ enc_w で θ を MVAE 潜在空間 z に写像（8次元）
         └─ MVAE 潜在空間 z 上でガウス尤度を解析的に計算（一瞬）
         ↓
【出力】θ = (k₁, k₂, k₃, k₄) の事後分布（2000サンプル、物理空間 [L,U] に逆変換済み）
        → 「真値はここらへんにある（± 不確実性）」という推定結果
```

> **2 種類の「空間」に注意**：MVAE の潜在空間 $z \in \mathbb{R}^8$（尤度評価の場）と、SMC のサンプル空間 $\theta_{\text{latent}} \in \mathbb{R}^4$（無制約空間。物理パラメータと CDF 変換で結ばれる）は別物。詳しくは [06_latent_likelihood.md §6](06_latent_likelihood.md) を参照。
