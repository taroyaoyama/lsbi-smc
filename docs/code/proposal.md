...
**SMC との相性**：温度 $\beta$ が上がるにつれて中間分布 $\pi_\beta$ は鋭くなり、粒子の散らばりも小さくなる。固定共分散だと序盤は小さすぎ、終盤は大きすぎる、という両端で失敗するが、Ching & Chen は **粒子集合自身の広がりに合わせて自動でスケールが追従** する（[07_smc.md §7.3](../07_smc.md)）。
