...
3つのガウス分布の積・商の積分が解析的に計算できることを利用。導出は 4 ステップに分かれる：

1. **正規化定数を被積分関数の外に括り出す** → 前因子 $d$ が現れる
2. **指数部分を $z$ の二次式 $-az^2 - bz - c$ として整理** → 係数 $a, b, c$ が決まる
3. **平方完成して標準ガウス積分に帰着** → $\sqrt{\pi/a}\cdot e^{b^2/(4a)}$
4. **対数をとり潜在次元の和をとる**

詳細は [06_latent_likelihood.md §3](../06_latent_likelihood.md) を参照。
...
| 変数 | 数式上の役割 | 由来 |
|------|------------|------|
| `a` | 指数二次項の係数 ($-az^2$) | 3 つのガウスの $1/(2\sigma^2)$ の符号付き和 |
| `b` | 指数一次項の係数 ($-bz$) | $\mu/\sigma^2$ の符号付き和 |
| `c` | 指数定数項 | $\mu^2/(2\sigma^2)$ の符号付き和 |
| `d` | **3 つのガウスの正規化定数を組合せた前因子** | $\sqrt{\sigma_3^2 / (2\pi \sigma_1^2 \sigma_2^2)}$ |

`d` は「3 つのガウスを正規化定数 × 指数項に分解したとき、$z$ に依存しない部分を積分の外に括り出した結果」として現れる（[06_latent_likelihood.md §3 Step 1](../06_latent_likelihood.md) 参照）。

#### ガウス積分の解析解

指数の中身 $-az^2 - bz - c$ を $z$ について平方完成すると：

$$-az^2 - bz - c = -a\!\left(z + \tfrac{b}{2a}\right)^2 + \frac{b^2}{4a} - c$$

これを標準ガウス積分 $\int e^{-au^2} du = \sqrt{\pi/a}$（$a > 0$ が必要）に帰着させて：

$$I = d \cdot e^{(b^2 - 4ac)/(4a)} \cdot \sqrt{\pi/a}$$
...
> **収束条件**: $a > 0$ が必要、つまり $1/\sigma_1^2 + 1/\sigma_2^2 > 1/\sigma_3^2$。本実装では $\sigma_3^2 = 1$（事前分布）で、エンコーダ出力の $\sigma_1^2, \sigma_2^2$ は通常十分小さいため、この条件は自然に満たされる。
